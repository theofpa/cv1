...
rotMat = [cos(theta), -sin(theta); sin(theta), cos(theta)]; % Done
end
...
cosCarrier = cos(2 * pi * (rot_x ./ lambda) + psi); % Done
...
sinCarrier = sin(2 * pi * (rot_x ./ lambda) + psi); % Done
...
gaussEnv = exp(-((rot_x.^2 + gamma^2 * rot_y.^2)/(2 * sigma^2))); % Done
